...
| 数据 | 接口提供的内容 |
| --- | --- |
| 几何 | 网格、曲线及曲面重建结果、源 B 样条曲线与底层曲面求值、复合曲线原生参考系查询、源放样侧面、封口区域与派生网格、Akima 与插值曲线转换、缓和曲线求值及 B 样条拟合、UV 裁剪区域查询与派生曲面网格、地形、线和文字、实例变换、图元范围、显式法线与 UV 及其独立索引 |
| 属性 | 属性树、二进制字段、对象身份、构件绑定、模型属性和对象关系 |
| 结构树 | 源结构树、模型树、原生模型目录及名称查找、父子关系、构件引用、块定义及嵌套块引用 |
| 材质 | 材质定义、参数、颜色表、材质候选、网格逐面材质引用与颜色通道、贴图路径引用和嵌入图片文件、显式上下文下的投影 UV 查询 |
| 图层与视图记录 | 图层表与模型引用、组合属性同步及来源、显式模型链接顺序 |
| 底层数据 | OLE 流、解压数据、DEX 对象、原生记录、图形命令及原始载荷 |
| CSG 数据 | 嵌套布尔树、几何与缓存列表、节点变换、原生索引关联、独立网格布尔求值、多面体自动三角化、箱体／四棱台、圆锥、球体／椭球体、环面管、拉伸及直纹扫掠派生网格，以及嵌套树更新与源面追溯；实体离散化的支持范围见已知限制 |
